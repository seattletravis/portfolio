...
gameInfo.addEventListener('mouseover', () => {
	gameInfo.className =
		'text-green-600 font-bold border-4 border-green-600 bg-blue-600 inline-block rounded-full px-6';
});
gameInfo.addEventListener('mouseout', () => {
	gameInfo.className =
		'text-green-600 font-bold border-4 border-green-600 bg-yellow-400 inline-block rounded-full px-6';
});
...
var panoMaterial = new THREE.MeshBasicMaterial({
	map: new THREE.TextureLoader().load('/tilt3d/tower_images/pano3.jpg'),
});
...
const tableTexture = new THREE.TextureLoader().load(
	'/tilt3d/tower_images/wood.jpg',
);
...
const wood = new THREE.TextureLoader().load(
	'/tilt3d/tower_images/topBottomA2.png',
);
...
function createBlock(blockName, blockPosition, blockShape, blockMeshArray) {
	...
	const roundedBoxGeometry = new THREE.BoxGeometry(
		blockShape.L * 2,
		blockShape.H * 2,
		blockShape.W * 2,
	);
...
window.addEventListener('pointerup', (event) => {
	...
	if (holdingTile == true) {
		...
		if (
			(redDroppability == true || blueDroppability == true) &&
			gameOver == false
		) {
			...
			averageScore.innerText =
				'Average: ' + Math.round(redsScore / currentRound) + ' Points';
